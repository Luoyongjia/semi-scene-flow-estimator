...
class WeightNet(nn.Module):
    def __init__(self, in_channel, out_channel, hidden_unit=[8, 8], bn=use_bn):
        """using n(len hidden_unit) layers to calculate the weights of point based on location.

        Args:
            hidden_unit (list, optional): out_channel of hidden layers. Defaults to [8, 8].
        """
        super(WeightNet).__init__()
        self.bn = bn
        self.convs = nn.ModuleList()
        self.bns = nn.ModuleList()

        if hidden_unit is None or len(hidden_unit) == 0:
            self.convs.append(nn.Conv2d(in_channel, out_channel, 1))
            self.bns.append(nn.BatchNorm2d(out_channel))
        else:
            self.convs.append(nn.Conv2d(in_channel, hidden_unit[0], 1))
            self.bns.append(nn.BatchNorm2d(hidden_unit[0]))
            
            for i in range(1, len(hidden_unit[-1], out_channel)):
                self.convs.append(nn.Conv2d(hidden_unit[i - 1], hidden_unit[i], 1))
                self.bns.append(nn.BatchNorm2d(hidden_unit[i]))
            
            self.convs.append(nn.Conv2d(hidden_unit[-1], out_channel), 1)
            self.bns.append(nn.BatchNorm2d(out_channel))
...
class PointConvD(nn.Module):
    def __init__(self, npoint, nsample, in_channel, out_channel, weightnet=16, bn=use_bn, use_leaky=True):
        """
        PointConv with downsample.

        Args:
            npoint (int): the number of output points
            nsample (int): the size of knn(scale) points
        """
    ...
    def forward(self, xyz, points):
        ...
        # use the furthest point sample to sample n points
...
class CostVolume(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        grouped_points2 = index_points_group(points2, knn_idx)
        grouped_points1 = points1.view(B, N1, 1, D1).repeat(1, 1, self.nsample, 1)
        new_points = torch.cat([grouped_points1, grouped_points2, direction_xyz], dim=-1)
        ...
        # weights
        weights = self.weightnet1(direction_xyz.permute(0, 3, 2, 1))
        point_to_patch_cost = torch.sum(weights * new_points, dim=2)

        # patch to patch
        knn_idx = knn_point(self.nsample, xyz1, xyz2)
...
class Warping(nn.Module):
    def forward(self, xyz1, xyz2, flow1=None):
        ...
        xyz1_to_xyz2 = xyz1 + flow1

        B, C, N1 = xyz1.shape
        _, _, N2 = xyz2.shape
        xyz1_to_xyz2 = xyz1_to_xyz2.permute(0, 2, 1)
        xyz2 = xyz2.permute(0, 2, 1)
        flow1 = flow1.permute(0, 2, 1)

        knn_idx = knn_point(3, xyz1_to_xyz2, xyz2)
        grouped_xyz_norm = index_points_group(xyz1_to_xyz2, knn_idx) - xyz2.view(B, N2, 1, C)
        dist = torch.norm(grouped_xyz_norm, dim=3).clamp(min=1e-10)
        norm = torch.sum(1.0 / dist, dim=2, keepdims=True)
